...
def main(argv: list[str]) -> None:  # pylint: disable=unused-argument
    ...
    resource_id = (
        ...
        else os.getenv("AGENT_RESOURCE_ID")
    )
    user_id = (
        FLAGS.user_id
        if FLAGS.user_id
        else os.getenv("DEFAULT_USER_ID")
    )
    ...
    if not project_id:
        ...
        print("Missing required environment variable: AGENT_RESOURCE_ID")
        return
    elif not user_id:
        print("Missing required environment variable: DEFAULT_USER_ID")
        return
